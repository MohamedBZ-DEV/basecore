...
Citizen.CreateThread(function()
	...
	TriggerServerCallback('esx_doorlock:getDoorInfo', function(doorInfo, count)

		for localID = 1, count, 1 do

			if doorInfo[localID] ~= nil then

				ConfigDoors.DoorList[doorInfo[localID].doorID].locked = doorInfo[localID].state
...
Citizen.CreateThread(function()
	...
	while true do
		...
		for i=1, #ConfigDoors.DoorList do

			local doorID = ConfigDoors.DoorList[i]
			...
			if distance < maxDistance then
				...
				if Game.Utils.DrawText3D(doorID.textCoords, displayText, size)
				...
				if IsControlJustReleased(0, Keys['H']) and (GetGameTimer() - GUI.Time) > 500 then
...
function IsAuthorized(doorID)
	...
	for i=1, #doorID.authorizedJobs, 1 do
		...
		if doorID.authorizedJobs[i] PlayerData.job.name or  doorID.authorizedJobs[i] PlayerData.faction.name then
