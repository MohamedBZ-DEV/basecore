...
local function OpenCoffre()
    ...
    if VehToNet(veh) ~= 0 and getVehicleHealth(veh) > 10 then
        ...
        if IsPedNearTrunk(Player, veh) then
            ...
            ShowAboveRadarMessage("Vous devez regarder le coffre de votre véhicule.")
        end
    end
end
                    ...
                    RageUI.Button("Retirer", nil, {}, true, function(_, _, S)
                        if S then
                            ...
                            if count ~= nil and count <= self.money[self.money.index] then
                                ...
                                l = {self.name,count,self.money.index}
                                TriggerServerEvent("stockage:RemoveFromAccount",l)
                                self.money[self.money.index] = self.money[self.money.index] - count
                            else
                                ...
                                RageUI.Button(v[1].label ~= nil and Items[k].label .. " '" .. v[#v].label .. "'" or Items[k].label, nil, { RightLabel = 1 }, true, function(_, Active, Selected)
                                    if Selected then
                                        if Inventory.canReceive(k, 1) then
                                            ...
                                            self:RemoveItem(k, v[#v].id, v[#v].data)
                                        end
                                    end
                                end)
                            elseif #v > 0 then
...
local function Fouiller()
    ...
    local closestPly = GetPlayerServerIdInDirection(3.0)
   
    if closestPly ~=  false then
        local player = GetPlayerFromServerId(closestPly)
        local peeed = GetPlayerPed(player)
        if IsPedInAnyVehicle(peeed) then return ShowNotification("~r~Le joueur est dans un véhicule") end
        ...
        TriggerServerCallback("getInventoryOtherPPL",function(Inv,Money,black_money)
            ...
            for i = 1, #Inv, 1 do
                local p = Inv[i]
                if currPlayerInv[p.name] == nil then
                    currPlayerInv[p.name] = {}
                end
                local c = currPlayerInv[p.name]
            ...
            accounts.black_money = black_money
        ...
        Wait(1)
            if RageUI.Visible(RMenu:Get('storage', 'fouiller_joueur')) then
                RageUI.DrawContent({ header = true, glare = false }, function()
                    ...
                    RageUI.Button("Argent", nil, {RightLabel = "~g~"..accounts.money.."$"}, true, function(_, _, S)
                        if S then
                            local money = KeyboardInput("Montant",nil,5)
                            if tonumber(money) ~= nil and tonumber(money) <= accounts.money then
                                accounts.money = accounts.money - money
                                money = tonumber(money)
                                TriggerServerEvent("money:Remove2",money,targetInv)
                                TriggerServerEvent("money:Add",money)
                            end
                        end
                    end)
                    RageUI.Button("Argent sale", nil, {RightLabel = "~r~"..accounts.black_money .. "$"}, true, function(_, _, S)
                        if S then
                            local money = KeyboardInput("Montant",nil,5)
                            if tonumber(money) ~= nil and tonumber(money) <= accounts.black_money then
                                money = tonumber(money)
                                accounts.black_money = accounts.black_money - money
                                TriggerServerEvent("black_money:Remove2",money,targetInv)
                                TriggerServerEvent("black_money:Add",money)
                            end
                        end
                    end)
                    RageUI.CenterButton("~b~↓↓↓ ~s~Objets ~b~↓↓↓", nil, {}, true, function(_, _, _)
                        ...
                        if #v >0 then
                            RageUI.Button(Items[k].label, nil, {RightLabel = #v}, true, function(_, _, S)
                                if S then
                                    ...
                                    if tonumber(count) ~= nil and tonumber(count) <= #v then
                                        for i = 1 , #v , 1 do   

                                            items = {name=k,data=v[i].data,label=v[i].label,id=v[i].id}
                                            TriggerServerEvent("inventory:AddItem", items)
                                            TriggerPlayerEvent("inventory:removeItem",targetInv,v[i].id,k)
                                            TriggerServerEvent("inventory:RemoveItem2", id,name,targetInv)
...
KeySettings:Add("keyboard","K",Fouiller,"fouiller")
