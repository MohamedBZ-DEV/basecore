...
AddEventHandler('core:NewAppartDuSexe', function(bool)
    ...
    TriggerClientEvent("core:SyncApparts2", -1,result)

end)
RegisterServerCallback('core:GetSKin', function(_source, callback)
   
    local ply = Player.GetPlayer(_source)

    MySQL.Async.fetchAll('SELECT * FROM players_appearance WHERE uuid =@name', {
        ["@name"] = ply.uuid,
