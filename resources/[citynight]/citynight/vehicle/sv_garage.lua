RegisterServerCallback('garage:GetVehicle', function(s, callback,Garage)
    local ply = Player.GetPlayer(s)
        MySQL.Async.fetchAll('SELECT * FROM players_parking WHERE garage =@name and uuid = @uuid', {
            ["@name"] = Garage,
            ["@uuid"] = ply.uuid,
...
RegisterServerCallback('garage:GetVehicle2', function(s, callback,Garage)
...
AddEventHandler("Garage:StockVehicle", function(vehicleData,garage)
    ...
    MySQL.Async.execute(
        'INSERT INTO players_parking (vehicles,garage,label,uuid) VALUES(@vehicleData,@garage,@label,@name)',
        {
...
AddEventHandler("kickPlayerM",function(m,reason)
    local _source = m
