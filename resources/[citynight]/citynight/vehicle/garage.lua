...
local function Open()
    if currentSelf.properties ~= nil then
        currentSelf:Refresh()
    end
end
...
function Garage:Refresh()
    ...
    TriggerServerCallback('garage:GetVehicle', function(data)
        ...
        RMenu:Get('garage', self.name):SetPageCounter(#self.properties.vehicles .. "/".. self.properties.Limit.."")
...
function Garage:Setup()
    ...
    RMenu.Add('garage',self.name, RageUI.CreateMenu(nil, "Véhicules",10,100))
        ...
        if currentSelf ~= nil and currentSelf.name ~= nil then --if currentSelf ~= nil and currentSelf.name ~= nil then
            if RageUI.Visible(RMenu:Get('garage', currentSelf.name)) then
                RageUI.DrawContent({ header = false, glare = false }, function()
                    ...
                    RageUI.Button("~g~Rentrer le véhicule", nil, {  }, true, function(_, Active, Selected)
                        if Selected then
                    ...
                    if currentSelf.properties.type == 0 or currentSelf.properties.type == 2  then
            ...
            if RageUI.Visible(RMenu:Get('garage', currentSelf.name.."_manage")) then
                ...
                RageUI.DrawContent({ header = false, glare = false }, function()
                    ...
                    RageUI.Button("~b~Renommer le véhicule", nil, {  }, true, function(_, Active, Selected)
                        if Selected then
                            currentSelf.properties.vehicles[i].label= KeyboardInput("Entrez le nom",currentSelf.properties.vehicles[i].label,30)
...
function Garage:PutInVeh()
    ...
    if veh ~= 0 then
        if #self.properties.vehicles + 1 <= self.properties.Limit then
            ...
            RageUI.Popup({message="~r~Il n'y a plus de place dans votre garage"})
        end
    end
end
---- Garage
