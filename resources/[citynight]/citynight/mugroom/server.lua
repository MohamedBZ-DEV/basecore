...
local function InsertIdentity(UUID, FacePicture, FirstName, LastName, BirthDate, Origin)
    MySQL.Async.execute('INSERT INTO players_identity (uuid, face_picutre, first_name, last_name, birth_date, origine) VALUES (@uuid, @face_picutre, @first_name, @last_name, @birth_date, @origine)', {
        ['@uuid'] = UUID,
...
local function InsertAppearance(UUID, Model, Face, Outfit, Tattoo)
    ...
    MySQL.Async.execute('INSERT INTO players_appearance (uuid, model, face, outfit, tattoo) VALUES (@uuid, @model, @face, @outfit, @tattoo)', {
        ['@uuid'] = UUID,
        ['@model'] = Model,
...
local function InsertJob(UUID, Name, Grade)
    ...
    MySQL.Async.execute('INSERT INTO players_jobs (uuid, name,rank) VALUES (@uuid, @name, @rank)', {
        ['@uuid'] = UUID,
        ['@name'] = Name,
        ['@rank'] = Grade,
        ['@created_at'] = os.date("%Y-%m-%d %H:%M:%S", os.time()),
...
function Database:RegisterPlayer(source, position, callback)
    TriggerEvent('rage-reborn:uuid', function(uuid)
        MySQL.Async.execute("INSERT INTO players (identifier, uuid, is_active, position) VALUES (@identifier, @uuid, @is_active, @position)", {
            ['@identifier'] = GetIdentifiers(source)[servers.identifier],
            ['@uuid'] = uuid,
            ['@is_active'] = 1,
...
function Database:RegisterJobs(uuid)
    MySQL.Async.execute("INSERT INTO players_jobs (uuid, jobs, rank_id, settings, permission) VALUES (@uuid, @jobs, @rank_id, @settings, @permission)", {
        ['@uuid'] = uuid,
        ['@jobs'] = "unemployed",
        ['@rank_id'] = 0,
...
function Database:RegisterOrganization(uuid)
    MySQL.Async.execute("INSERT INTO players_organization (uuid, name, rank_id, settings, permission) VALUES (@uuid, @name, @rank_id, @settings, @permission)", {
        ['@uuid'] = uuid,
        ['@name'] = nil,
        ['@rank_id'] = 0,
        ['@settings'] = json.encode({}),
...
function Database:RegisterIdentity(uuid, first_name, last_name, birth_date, origine)
    MySQL.Sync.execute("INSERT INTO players_identity (uuid, face_picutre, first_name, last_name, birth_date, origine) VALUES (@uuid, @face_picutre, @first_name, @last_name, @birth_date, @origine)", {
        ['@uuid'] = uuid,
...
function Database:RegisterAppearance(uuid, model, face, outfit, tattoo)
    MySQL.Sync.execute("INSERT INTO players_appearance (uuid, model, face, outfit, tattoo) VALUES (@uuid, @model, @face, @outfit, @tattoo)", {
        ['@uuid'] = uuid,
        ['@model'] = model,
        ['@face'] = json.encode(face),
        ['@outfit'] = json.encode(outfit),
...
function Database:RegisterEconomy(uuid, money, dirty_money)
    MySQL.Sync.execute("INSERT INTO players_economy (uuid, money, dirty_money) VALUES (@uuid, @money, @dirty_money)", {
        ['@uuid'] = uuid,
        ['@money'] = money,
...
function Database:RegisterNeeds(uuid)
    ...
    MySQL.Sync.execute("INSERT INTO players_needs (uuid, content, health, armour) VALUES (@uuid, @content, @health, @armour)", {
        ['@uuid'] = uuid,
    ...
    MySQL.Async.fetchAll('SELECT * FROM storages_inventory_items WHERE name = @name', {
        ...
        MySQL.Async.fetchAll('SELECT * FROM storages_inventory_accounts WHERE name = @name', {
            ...
            if _result[1] == nil then
                MySQL.Async.execute(
                    'INSERT INTO storages_inventory_accounts (name) VALUES(@name)',
                    {
                        ['@name']   =name,
                        ['@created_at'] = GetDatetime(),
                        ['@updated_at'] = GetDatetime(),
