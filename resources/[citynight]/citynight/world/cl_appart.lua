...
AddEventHandler("add:Appart",function(_table)
    local _table = json.decode(_table)
    _table.entry = json.decode(_table.entry)
    _table.entry.z = _table.entry.z
    table.insert(Property,{id=math.random(111,333333),indexx=_table.index,name=_table.name,owner=nil,price=_table.price,capacity=_table.max,pos=_table.entry})
...
function NoOwnerAppart()
    ...
    RageUI.Button("Contacter un agent immobilier",nil,{RightLabel = ""},true,function(_,_,Selected)
        if Selected then
            no("~g~Vous avez appellé un agent immobilier")
            MakeCall("immo")
        end
    end)
