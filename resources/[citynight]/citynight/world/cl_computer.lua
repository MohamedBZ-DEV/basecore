...
local Items = {
    ...
    {item="knuckle",data=json.encode({serial=math.random(111111111,999999999)}),chance=20},
    {item="bottle",data=json.encode({serial=math.random(111111111,999999999)}),chance=8},
    {item="crowbar",data=json.encode({serial=math.random(111111111,999999999)}),chance=20},
    {item="gants",chance=10},
    {item="cana",chance=20},
        ...
        exports['mythic_progbar']:Progress({
            ...
            if not cancelled then
                ...
                if random2 > 4 then
                        for i = 1 , #Items , 1 do
                            if random ==  Items[i].chance then
                                ...
                                if Inventory.CanReceive(Items[i].item,1) then
                                    RageUI.Popup({message="~g~Vous avez trouvé 1" .. string.lower(_Items[Items[i].item].label) .."~s~ dans la poubelle"})
                        ...
                        if not found then
                            for i = 1 , #Items , 1 do
                                if 50 <=  Items[i].chance then
                                    found = true
                                    if Inventory.CanReceive(Items[i].item,1) then
                                        RageUI.Popup({message="~b~Vous avez trouvé 1" .. string.lower(_Items[Items[i].item].label) .."~s~ dans la poubelle"})
                                        local items = {name=Items[i].item,data=Items[i].data}
                                        TriggerServerEvent("inventory:AddItem", items)
                    ...
                    RageUI.Popup({message="~r~Vous n'avez rien trouvé"})
                end
            else
...
local distributor = {
	["prop_vend_coffe_01"] = { it = "cafe", max = 1 },
