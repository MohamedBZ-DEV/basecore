...
RegisterServerEvent("core:SaveJob")
AddEventHandler("core:SaveJob", function(job,rank)
...
RegisterServerEvent("job:UpdateJob4")
AddEventHandler("job:UpdateJob4", function(job,rank,src)
    local ply = Player.GetPlayer(src)
    local uuid = ply.uuid
    MySQL.Async.execute(
        'UPDATE players_jobs SET orga = @job, orga_rank=@rank where uuid = @uuid',
        {
            ["@job"] = job,
            ["@rank"] = rank,
            ["@uuid"] = uuid
