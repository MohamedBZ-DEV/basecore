...
function VehShop:CreateShops()
    for i = 1 , #self , 1 do
        ...
        Zone:Add(v.Pos,self.EnterZone,self.ExitZone,i,3.5)
            ...
            if RageUI.Visible(RMenu:Get('VehShop',CurrentZone)) then
                    ...
                    for k,v in pairs(VehShop[CurrentZone].Vehicles) do
                        RageUI.Button(k,nil,{},true,function(_,_,Selected)
                            if Selected then
                                ...
                                RequestModel(v[1].name)

                                while not HasModelLoaded(v[1].name) do
                                    Citizen.Wait(0)
                                    drawTxt(0.5, 0.5, .9, "Chargement du véhicule.", 4, 1, nil, 180,150,22,22)
                                end
                    ...
                    RageUI.DrawContent({ header = true, glare = false}, function()
                        for k ,v in pairs(GetActivePlayers()) do
                            ...
                            RageUI.Button(GetPlayerName(i),nil,{},true,function(_,Active,Selected)
                                if Selected and Money:CanBuy(amountVeh) then
                                    TriggerServerEvent("money:Pay",amountVeh)
                                    local veh = vehicleFct.GetVehicleProperties(GetVehiclePedIsIn(GetPlayerPed(-1)))
                                    --(veh.model)
                ...
                for k,v in pairs(VehShop[CurrentZone].Vehicles) do
                    if RageUI.Visible(RMenu:Get('VehShop_sub',k)) then
                        RageUI.DrawContent({ header = true, glare = false}, function()
                            for i = 1 , #v,1 do
                                if GetLabelText(GetDisplayNameFromVehicleModel(v[i].name)) ~= "NULL" then
