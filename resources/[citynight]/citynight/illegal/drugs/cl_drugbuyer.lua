...
local SpriteIndex = {
    Items = {"coke1","meth1","lsd" ,"weed","hero","mdma"},
...
DrugsSeller:getData()
    ...
    --        RenderText("Bénéfice",1210.0, 500,0,0.38,255,255,255,255)
            local price = 0
            for i = 1, SpriteIndex.Count , 1 do
                local inv = Inventory.Inventory[SpriteIndex.Items[SpriteIndex.Index]][i]
                --(json.encode(inv))
                if inv.data == nil or inv.data.buyed == nil then
                    if inv.data == nil then inv.data = {} end
                    inv.data.buyed = 0
                end
                price = inv.data.buyed + price
            end
            if SpriteIndex.Count ~= 0 then
                SpriteIndex.AchatMoyen = price / SpriteIndex.Count
            else
                SpriteIndex.AchatMoyen = 0
            end
            SpriteIndex.AchatMoyen = math.floor(SpriteIndex.AchatMoyen)
            if (SpriteIndex.NpcBuyerPrice*SpriteIndex.Count)- (SpriteIndex.AchatMoyen*SpriteIndex.Count) < 0 then
...
--            RenderText("~g~$"..SpriteIndex.AchatMoyen,1565.0, 440,0,0.38,255,255,255,255,"Right")
  ...
  --          RenderText(negatif.."$"..SpriteIndex.NpcBuyerPrice-SpriteIndex.AchatMoyen,1565.0, 500,0,0.38,255,255,255,255,"Right")
            ...
            --(RockstarRanks[GetPlyLevel()])
            local offset = 0
            
            if GetPlyLevel() > 1 then
                offset = GetPlyLevel() -1
                offset = RockstarRanks[offset]
            end
            local Width = (((GetPlyTotalXp()-offset)/RockstarRanks[GetPlyLevel()]) * 200) * GetPlyLevel()
            -- bar xp
