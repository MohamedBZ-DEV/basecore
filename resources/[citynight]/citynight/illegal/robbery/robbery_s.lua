...
AddEventHandler('fx_bank:rob', function(robb)
	if banks[robb] then
		...
		if (os.time() - bank.lastrobbed) < 600 and bank.lastrobbed ~= 0 then
			TriggerClientEvent('chatMessage', source, '[ROBBERY]', {255, 0, 0}, "Cela a déjà été volé récemment. Veuillez attendre un autre: ^2" .. (1200 - (os.time() - bank.lastrobbed)) .. "^0 seconds.")
			return
		end
		TriggerClientEvent('chatMessage', source, '^7[^1ROBBERY^7]', {255, 0, 0}, "Vous avez commencé un vol a:^ 1" .. bank.nameofbank .. "^ 0, ne t'éloigne pas trop de ce point!")
