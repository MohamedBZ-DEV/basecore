...
AddEventHandler('fx_bank:sendalert', function(name)
	if IsCop then
	TriggerEvent('chatMessage', '^7[^4Dispatch^7]', {255, 0, 0}, "Vol en cours à^1" .. name)		
	end
end)
...
AddEventHandler('fx_bank:toofarlocal', function(robb)
	robbing = false
	TriggerEvent('chatMessage', '[ROBBERY]', {255, 0, 0}, "Le vol a été annulé, vous ne recevrez rien.")
	robbingName = ""
	secondsRemain = 0
	incircle = false
end)
...
AddEventHandler('fx_bank:robberycomplete', function(robb)
	robbing = false
	TriggerEvent('chatMessage', '[ROBBERY]', {255, 0, 0}, "Le vol est fait! L'argent ajouté à votre argent.")
	bank = ""
	secondsRemain = 0
	incircle = false
end)
...
Citizen.CreateThread(function()
	while true do
				...
				if not robbing then
					...
					if(Vdist(pos.x, pos.y, pos.z, pos2.x, pos2.y, pos2.z) < 1.0)then
						if (incircle == false) then
							DisplayHelpText("Presse ~INPUT_CONTEXT~ voler ~b~" .. v.nameofbank .. "~w~ attention, la police sera alertée!							")
						end
						incircle = true
		...
		if robbing then
			...
			drawTxt(0.66, 1.42, 1.0,1.0,0.4, "Lieu de vol: ~r~" .. secondsRemain .. "~w~ secondes restantes", 255, 255, 255, 255)
