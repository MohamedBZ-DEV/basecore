...
AddEventHandler("stockage:AddMoneyToAccount", function(plate,money,type)
    ...
    if type == "dark_money" then
        player.removeBlackMoney(money)
        MySQL.Async.execute(
            'UPDATE storages_inventory_accounts SET dark_money = @money + dark_money where name=@plate',
               {
                    ['@money']   = money,
                    ['@plate'] = plate,
                }
...
AddEventHandler("stockage:RemoveFromAccount", function(plate,money,type)
    ...
    local player = Player.GetPlayer(source)
    if plate[3] == "dark_money" then
        
        MySQL.Async.execute(
            'UPDATE storages_inventory_accounts SET dark_money =  dark_money - @money where name=@plate',
               {
                    ['@money']   = plate[2],
                    ['@plate'] = plate[1],
                }
